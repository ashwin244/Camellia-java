 ...
 *
 */
public class Main_Integer_Output {

	private static final long MASK8 = 0xffL;
	private static final long MASK32 = 0xffffffffL;
	private static enum KeyType { KEY_128, KEY_192, KEY_256};
	private static final long[] Sigma = {
		   0xA09E667FL, 0x3BCC908BL,
		   0xB67AE858L, 0x4CAA73B2L,
		   0xC6EF372FL, 0xE94F82BEL,
		   0x54FF53A5L, 0xF1D36F1CL,
		   0x10E527FAL, 0xDE682D1DL,
		   0xB05688C2L, 0xB3E6C1FDL};
	
	private static final long SBOX1[] = { 112, 130, 44, 236, 179, 39, 192, 229,
	...
	private static long[] KL = new long[4];     // Encryption key
	private static long[] KR = new long[4];     // Encryption key
	private static final long[] K = new long[8];     // Encryption key
	private static long[] k = new long[24 * 2];
	private static long[] kDec = new long[24 * 2];
	private static long[] KA = new long[4];     
	private static long[] KB = new long[4];
	
	
	private static long[] kw = new long[4 * 2];
	private static long[] ke = new long[6 * 2];
	
	private static long[] kwDec = new long[4 * 2];
	private static long[] keDec = new long[6 * 2];

	public static long[] Xor128(long[] left, long[] right){
	...
	public static long[] Xor64(long[] left, long[] right, int offset){
    ...
    private final static long bytes2long(int[] src, int offset) {
	long word=0;
    ...
    private final static void long2bytes(long word, int[] dst, int offset) {
	for(int i = 0; i< 4; i++) {
	    dst[(3-i)+offset] = (int) word;
	    word >>>= 8;
	}
    }
	...
	//key scheduling part
	private static void keySchedule(int[] key, long keyType) {
		if (keyType == KeyType.KEY_128.ordinal()) {
			
		    K[0] = bytes2long(key, 0);
		    K[1] = bytes2long(key, 4);
		    K[2] = bytes2long(key, 8);
		    K[3] = bytes2long(key, 12);
		    K[4] = K[5] = K[6] = K[7] = 0;
		}
		if (keyType == KeyType.KEY_192.ordinal()) {
		    ...
		    K[2] = bytes2long(key, 8);
		    K[3] = bytes2long(key, 12);
		    K[4] = bytes2long(key, 16);
		    K[5] = bytes2long(key, 20);
		    K[6] = ~K[4];
		    K[7] = ~K[5];
		}
		if (keyType == KeyType.KEY_256.ordinal()) {
		    ...
		    K[4] = bytes2long(key, 16);
		    K[5] = bytes2long(key, 20);
		    K[6] = bytes2long(key, 24);
		    K[7] = bytes2long(key, 28);
		}
		...
		//KA and KB
		   ...
		   rotate64(45, KL, k, 12);
			...
			k[16] = (KL[1] << (45 - 32)) & MASK32 | (KA[2] >>> (64 - 45- 1));
			k[17] = (KL[2] << (45 - 32)) & MASK32 | (KA[3] >>> (64 - 45- 1));
			
			k[18] = (KA[3] << (60 - 32)) & MASK32;
			k[19] = 0;
	...
	private static long[] F(long[] Fin, long[] KE, int offset){
		long[] x = new long[2];  
		long t1, t2, t3, t4, t5, t6, t7, t8;
		long y1, y2, y3, y4, y5, y6, y7, y8;
		long[] F_OUT =  new long[2];
		...
		x[1] = Fin[1] ^ KE[1 + offset];
	       ...
	       t1 =  SBOX1[(int) t1];
	       t2 = (SBOX1[(int) t2] << 1)& MASK8; //SBOX2[t2];
	       t3 = (SBOX1[(int) t3] << 7)& MASK8; //SBOX3[t3];
	       t4 = (SBOX1[(int) ((t4 << 1) & MASK8)])& MASK8; //SBOX4[t4];
	       t5 = (SBOX1[(int) t5] << 1)& MASK8; //SBOX2[t5];
	       t6 = (SBOX1[(int) t6] << 7)& MASK8; //SBOX3[t6];
	       t7 = (SBOX1[(int) ((t7 << 1) & MASK8)])& MASK8; //SBOX4[t7];
	       t8 =  SBOX1[(int) t8];
	...
	private static long[] dataRandomizingPart (int[] message, long keyType){
		
		long[] D1 = new long[2];
		long[] D2 = new long[2];
		long[] cipher = new long[4];
		D1[0] = bytes2long(message, 0);
		D1[1] = bytes2long(message, 4);
		D2[0] = bytes2long(message, 8);
		D2[1] = bytes2long(message, 12);
	...
	private static long[] FL(long[] FLin, long[] KE, long offset){
		long x1, x2;
		long k1, k2;
		long[] FL_OUT = new long[2];
	       x1 = FLin[0];
	       x2 = FLin[1];
	       k1 = KE[0];
	       k2 = KE[1];
	...
	private static long[] FLINV(long[] FLINVin, long[] KE, long offset){
		long y1, y2;
		long k1, k2;
		long[] FLINV_OUT = new long[2]; 
			y1 = FLINVin[0];
	       y2 = FLINVin[1];
	       k1 = KE[0];
	       k2 = KE[1];
	...
	private static final void rotate32(long rot, long[] ki, long[] ko, int offset) {
		ko[0 + offset] = (ki[0] << rot)& MASK32 | (ki[1] >>> (32 - rot));
		ko[1 + offset] = (ki[1] << rot) & MASK32 | (ki[2] >>> (32 - rot));
		ko[2 + offset] = (ki[2] << rot) & MASK32 | (ki[3] >>> (32 - rot));
		ko[3 + offset] = (ki[3] << rot) & MASK32;
	}
    
	private static final void rotate64(long rot, long[] ki, long[] ko, int offset) {
		ko[0 + offset] = (ki[1] << (rot - 32)) & MASK32 | (ki[2] >>> (64 - rot));
		ko[1 + offset] = (ki[2] << (rot - 32)) & MASK32 | (ki[3] >>> (64 - rot));
		ko[2 + offset] = (ki[3] << (rot - 32)) & MASK32;
		ko[3 + offset] = 0;
	}
	
	private static final void rotate96(long rot, long[] ki, long[] ko, int offset) {
		ko[0 + offset] = (ki[2] << (rot - 64)) & MASK32 | (ki[3] >>> (96 - rot));
		ko[1 + offset] = (ki[3] << (rot - 64)) & MASK32;
		ko[2 + offset] = 0;
		ko[3 + offset] = 0;
	}
	
	private static final void rotate128(long rot, long[] ki, long[] ko, int offset) {
		ko[2 + offset] = (ki[3] << (128 - rot)) & MASK32;
		ko[3 + offset] = 0;
		ko[0 + offset] = 0;
		ko[1 + offset] = 0;
	}
	...
	public static void init(int[] key, int[] message, long keyType){
		keySchedule(key, keyType);
		long[] cipher = dataRandomizingPart(message, keyType);
		int[] dst = new int[32];
		int offset = 0;
		for(long c : cipher){
			long2bytes(c, dst,offset); 
			offset +=4;
		}
	    ...
	    kDec = k;
	       ...
	       long[] decryptMessage = dataRandomizingPart(dst, keyType);
	       int[] dst1 = new int[32];
	       offset = 0;
			for(long d : decryptMessage){
				long2bytes(d, dst1, offset); 
				offset +=4;
			}
			System.out.println("\ndec message");
			for (int d: dst1){
	...
	public static void main(String[] args){
		...
		int[] msg = {0x01, 0x23, 0x45, 0x67,  0x89,  0xab,  0xcd,  0xef, 
				 0xfe,  0xdc,  0xba,  0x98, 0x76, 0x54, 0x32, 0x10};
